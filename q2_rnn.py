...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        ## it is possible the sentence is longer than max length, in that case use up to the max length
        pad_sentence = sentence[:max_length] + max(max_length - len(sentence), 0) * [zero_vector]
        pad_label = labels[:max_length] + max(max_length - len(labels), 0) * [zero_label]
        mask = [True] * min(max_length,len(labels)) + [False] * max(max_length - len(labels), 0)
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.int32,shape=(None,self.max_length),name='label')
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings = tf.reshape(tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings,tf.float32),self.input_placeholder),(-1,self.max_length,Config.n_features*Config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(name='U', shape=(Config.hidden_size,Config.n_classes), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss = tf.reduce_mean(tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds,labels=self.labels_placeholder),self.mask_placeholder))
        ### END YOUR CODE
        return loss
