...
class Config:
    ...
    ### YOUR CODE HERE
    n_window_features = n_word_features * (1 + 2 * window_size) # The total number of features used for each window.
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            ## work out the right amount of start and end to pre/append, and the number of elements from sentence
            f = [start] * abs(min(i - window_size,0)) + sentence[max(i - window_size,0):min(i + window_size + 1,len(sentence))] + [end] * max(i + window_size + 1 - len(sentence),0)
            ## flattern sublists
            f = [item for sublist in f for item in sublist]
            ## append a tuple
            windowed_data.append((f,labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings = tf.reshape(tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings,tf.float32),self.input_placeholder),(-1,Config.n_window_features*Config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name='W', shape=(Config.n_window_features*Config.embed_size,Config.hidden_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=1))
        b1 = tf.get_variable(name='b1', shape=(Config.hidden_size,), dtype=tf.float32, initializer=tf.zeros_initializer())
        
        U = tf.get_variable(name='U', shape=(Config.hidden_size,Config.n_classes), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=3))
        b2 = tf.get_variable(name='b2', shape=(Config.n_classes,), dtype=tf.float32, initializer=tf.zeros_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(self.labels_placeholder,self.config.n_classes),logits=pred))
