...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable("W_r", shape=(self.input_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_r = tf.get_variable("U_r", shape=(self.state_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_r = tf.get_variable("b_r", shape=(self.state_size,), initializer=tf.zeros_initializer(), dtype=np.float32)
            W_z = tf.get_variable("W_z", shape=(self.input_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_z = tf.get_variable("U_z", shape=(self.state_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_z = tf.get_variable("b_z", shape=(self.state_size,), initializer=tf.zeros_initializer(), dtype=np.float32)
            W_o = tf.get_variable("W_o", shape=(self.input_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            U_o = tf.get_variable("U_o", shape=(self.state_size,self.state_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=np.float32)
            b_o = tf.get_variable("b_o", shape=(self.state_size,), initializer=tf.zeros_initializer(), dtype=np.float32)
            
            z = tf.nn.sigmoid(tf.matmul(inputs,W_z) + tf.matmul(state,U_z) + b_z)
            r = tf.nn.sigmoid(tf.matmul(inputs,W_r) + tf.matmul(state,U_r) + b_r)
            o = tf.nn.tanh(tf.matmul(inputs,W_o) + r * tf.matmul(state,U_o) + b_o)
